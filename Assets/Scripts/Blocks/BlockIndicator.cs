    ...
    void Update()
    {
        ...
        if (inReach && hasMultitile)
        {
            ...
            if (def is DoorDefinition doorDef)
            {
                ...
                if (canPlace)
                {
                    _placeSr.sprite = doorDef.openSprite;
                    _placeSr.flipX = facingRight;
        ...
        if (hasTool && inReach && cellHasObject)
        {
            ...
            if (obj != null)
            {
                ...
                {
                    var size = doorObj.CurrentSize;
                    var origin = doorObj.CurrentOrigin;
                    _breakGo.transform.position = new Vector3(
                        origin.x + size.x * 0.5f,
                        origin.y + size.y * 0.5f);
                    _breakGo.transform.localScale = new Vector3(size.x * s, size.y * s, 1f);
                }
                else
                {
                    _breakGo.transform.position = new Vector3(
