...
public class WallBreakSystem : MonoBehaviour
{
    public static WallBreakSystem Instance { get; private set; }
    ...
    public bool TryBreak(Vector3Int cell, float deltaTime)
    {
        ...
        if (isTool && toolType != ToolType.None && toolType == data.requiredTool)
    ...
    {
        var wallType = WorldManager.Instance.GetWall(cell.x, cell.y);
        if (wallType == WallType.None) return 0f;
